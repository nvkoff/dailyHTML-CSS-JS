...
export function RevisionRunner({ questions }: { questions: Question[] }) {
  const [index, setIndex] = useState(0);
  const [answer, setAnswer] = useState<Answer>(null);
  const [phase, setPhase] = useState<Phase>("answering");
  const [correctCount, setCorrectCount] = useState(0);
  const [wasCorrect, setWasCorrect] = useState<boolean | null>(null);
  const [summary, setSummary] = useState<{ newStreak: number; totalXp: number } | null>(
    null,
  );
  ...
  if (!question && phase !== "done") {
    return (
      <div className="mx-auto max-w-md py-20 text-center text-muted-foreground">
        No revision questions available yet.
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-6 px-4 py-16 text-center sm:py-20">
        ...
        <div className="space-y-1">
          <h1 className="text-xl font-semibold sm:text-2xl">Revision complete</h1>
        ...
        <div className="flex w-full flex-col gap-2 sm:flex-row">
          ...
          <Button asChild className="flex-1">
            <Link href="/learn/revision">Another round</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-5 px-3 py-5 sm:gap-6 sm:px-4 sm:py-8">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        Revision — random questions from every topic
      </div>
